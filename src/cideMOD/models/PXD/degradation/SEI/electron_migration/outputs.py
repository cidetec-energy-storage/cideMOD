...
class MigrationSEIModelOutputs(BasePXDModelOutputs):
    """
    ...
    def get_outputs_info(self, warehouse: Warehouse) -> None:
        ...
        header = "Porous SEI {i} thickness [m]"
        warehouse.add_global_variable_info('delta_porous_sei_a', fnc=self.get_L_porous_sei,
                                           default=True, dtype='list_of_scalar', header=header)
        header = "Compact SEI {i} thickness [m]"
        warehouse.add_global_variable_info('delta_compact_sei_a', fnc=self.get_L_compact_sei,
                                           default=True, dtype='list_of_scalar', header=header)
        header = "Capacity loss to compact SEI {i} [Ah]"
        warehouse.add_global_variable_info('Q_sei_a', fnc=self.get_Q_sei,
                                           default=True, dtype='list_of_scalar', header=header)

        # Internal variables
        warehouse.add_internal_variable_info('delta_porous_sei_a', subdomains='anode',
                                             dtype='list_of_scalar', default=True)
        warehouse.add_internal_variable_info('delta_compact_sei_a', subdomains='anode',
                                             dtype='list_of_scalar', default=True)
    ...
    def prepare_outputs(self, warehouse: Warehouse, var: ProblemVariables, cell: BatteryCell,
        ...
        self.dx = getattr(problem.mesher, 'dx_a')
        self._j_instant_sei_forms = []
        self._delta_compact_sei_forms, self._delta_porous_sei_forms = [], [],
        for k, am in enumerate(cell.anode.active_materials):
            ...
            self._delta_porous_sei_forms.append(
                dfx.fem.form(var.f_1(f'delta_porous_sei_a{k}') * self.dx))
            self._delta_compact_sei_forms.append(
                dfx.fem.form(var.f_1(f'delta_compact_sei_a{k}') * self.dx))

        # Prepare internal variables
        warehouse.setup_internal_variable('j_sei_a', var.j_Li_a.SEI,
                                          length=var.n_mat_a)
        warehouse.setup_internal_variable('delta_porous_sei_a', var.delta_porous_sei_a,
                                          length=var.n_mat_a)
        warehouse.setup_internal_variable('delta_compact_sei_a', var.delta_compact_sei_a,
                                          length=var.n_mat_a)
    ...
    def get_L_porous_sei(self):
        return [self.problem.get_avg(form, self.dx)
                for form in self._delta_porous_sei_forms]

    def get_L_compact_sei(self):
        return [self.problem.get_avg(form, self.dx)
                for form in self._delta_compact_sei_forms]
