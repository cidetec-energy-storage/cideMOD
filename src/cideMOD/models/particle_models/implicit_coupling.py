...
class SpectralLegendreModel(StrongCoupledPM):
    ...
    def _get_domain(self, electrode):
        allowed = ('anode', 'cathode')
        if electrode not in allowed:
            raise ValueError("Argument electrode must be either 'anode' or 'cathode'")
    ...
    def fields(self, n_mat, electrode):
        ...
        return ['c_s_{}_{}{}'.format(order, domain, material)
                for material in range(n_mat) for order in range(self.order)]

    def initial_guess(self, f_0, electrode, c_s_ini, fs_mapper):
        domain = self._get_domain(electrode)
        n_mat = self._get_n_mat(f_0, domain)
        if n_mat != 0:
            assert isinstance(c_s_ini, (list, tuple)), "Keyword 'c_s_ini' must be an iterable"
            assert len(c_s_ini) == n_mat, f"Keyword 'c_s_ini' must be a list of length {n_mat}"
            for k, c_ini in enumerate(c_s_ini):
                fs_mapper.interpolate({electrode: c_ini}, f_0(f'c_s_0_{domain}{k}'), clear=True)
    ...
    def wf_implicit_coupling(self, f_0, f_1, test, electrode,
                             dx: Measure, DT, materials: List, F, R):
        domain = self._get_domain(electrode)
        F_c_s_ret = []
        for k, material in enumerate(materials):
            ...
            if 'temp' in f_1.var_names:
                D_s_eff *= exp(material.D_s_Ea * (1 / material.D_s_Tref - 1 / f_1.temp) / R)
            for j in range(self.order):
                F_c_s = 0
                F_c_s += (self.M[0, j] * DT.dt(f_0[c_s_index], f_1[c_s_index])
                          * test[c_s_index + j] * dx(metadata={"quadrature_degree": 2}))
                for i in range(1, self.order):
                    F_c_s -= (self.M[0, j] * DT.dt(f_0[c_s_index + i], f_1[c_s_index + i])
                              * test[c_s_index + j] * dx(metadata={"quadrature_degree": 2}))

                for i in range(1, self.order):

                    F_c_s += (self.M[i, j] * DT.dt(f_0[c_s_index + i], f_1[c_s_index + i])
                              * test[c_s_index + j] * dx(metadata={"quadrature_degree": 2}))

                    F_c_s += ((D_s_eff / material.R_s ** 2) * self.K[i, j] * f_1[c_s_index + i]
                              * test[c_s_index + j] * dx(metadata={"quadrature_degree": 2}))

                F_c_s += ((1. / material.R_s) * (1. / F) * self.P[j] * f_1[j_li_index]
                          * test[c_s_index + j] * dx(metadata={"quadrature_degree": 2}))
    ...
    def wf_explicit_coupling(self, f_0, f_1, test, electrode, dx):
        ...
        for material in range(n_mat):
            ...
            for j in range(0, self.order):
                F_c_s_aux.append(f_1[c_s_index + j] * test[c_s_index + j] * dx
                                 - f_0[c_s_index + j] * test[c_s_index + j] * dx)
        return F_c_s_aux
    ...
    def c_s_surf(self, f, electrode):
        ...
        c_s_surf = []
        for material in range(n_mat):
            c_s_surf_am = f(f'c_s_0_{domain}{material}')
            c_s_surf.append(c_s_surf_am)
        return c_s_surf
    ...
    def c_s_r_average(self, f, electrode):
        ...
        c_s_r_average = []
        for material in range(n_mat):
            c_s_r_average_am = weights[0] * f(f'c_s_0_{domain}{material}')
            for i in range(1, self.order):
                c_s_r_average_am += (weights[i] - weights[0]) * f(f'c_s_{i}_{domain}{material}')
    ...
    def Li_amount(self, f, electrode, materials: List, dx, volume_factor=1):
        ...
        for k, material in enumerate(materials):
            ...
            c_s_total = dfx.fem.assemble_scalar(
                volume_factor * particle_total * material.eps_s * dx)
...
class NondimensionalSpectralModel(SpectralLegendreModel):
    def wf_implicit_coupling(self, f_0, f_1, test, electrode,
                             dx: Measure, DT, materials: List, nd_model):
        domain = self._get_domain(electrode)
        F_c_s_ret = []
        for k, material in enumerate(materials):
            ...
            T = nd_model.unscale_variable('T', f_1.temp)
            if material.D_s_Ea == 0:
                D_s_eff = self.get_value(material.D_s, material.index, f_1, electrode)
            else:
                D_s_eff = (self.get_value(material.D_s, material.index, f_1, electrode)
                           * exp(material.D_s_Ea * (1 / material.D_s_Tref - 1 / T)
                                 / nd_model.cell.R))
            for j in range(self.order):
                F_c_s = 0
                F_c_s += (1 / mat_params['tau_s'] * self.M[0, j]
                          * DT.dt(f_0[c_s_index], f_1[c_s_index])
                          * test[c_s_index + j] * dx(metadata={"quadrature_degree": 2}))
                for i in range(1, self.order):
                    F_c_s -= (1 / mat_params['tau_s'] * self.M[0, j]
                              * DT.dt(f_0[c_s_index + i], f_1[c_s_index + i])
                              * test[c_s_index + j] * dx(metadata={"quadrature_degree": 2}))
                for i in range(1, self.order):

                    F_c_s += (1 / mat_params['tau_s'] * self.M[i, j]
                              * DT.dt(f_0[c_s_index + i], f_1[c_s_index + i])
                              * test[c_s_index + j] * dx(metadata={"quadrature_degree": 2}))

                    F_c_s += (D_s_eff / mat_params['D_s_ref'] * self.K[i, j] * f_1[c_s_index + i]
                              * test[c_s_index + j] * dx(metadata={"quadrature_degree": 2}))

                F_c_s += (mat_params['S'] * self.P[j] * f_1[j_li_index]
                          * test[c_s_index + j] * dx(metadata={"quadrature_degree": 2}))
                F_c_s_ret.append(F_c_s)
...
class StressEnhancedSpectralModel(SpectralLegendreModel):
    ...
    def theta(self, material, R, T):
        if None not in [material.omega, material.young, material.poisson]:
            return (material.omega * 2 * material.young * material.omega
                    / (R * 9 * (1 - material.poisson)) / T)
        else:
            raise RuntimeError(f"Material {material.index} does not have mechanical properties")
    ...
    def wf_implicit_coupling(self, f_0, f_1, test, electrode, dx: Measure, DT, materials, F, R):
        ...
        for k, material in enumerate(materials):
            ...
            D_s_eff = (self.get_value(material.D_s, material.index, f_1, electrode)
                       * exp(material.D_s_Ea * (1 / material.D_s_Tref - 1 / f_1.temp) / R))
            for j in range(self.order):
                F_c_s = 0
                for i in range(self.order):
                    F_c_s += (self.M[i, j] * DT.dt(f_0[c_s_index + i], f_1[c_s_index + i])
                              * test[c_s_index + j] * dx(metadata={"quadrature_degree": 2}))
                    F_c_s += ((1 - theta * material.c_s_ini) * (D_s_eff / material.R_s ** 2)
                              * self.K[i, j] * f_1[c_s_index + i]
                              * test[c_s_index + j] * dx(metadata={"quadrature_degree": 2}))
                    for l in range(self.order):
                        F_c_s += (theta * (D_s_eff / material.R_s ** 2) * self.S[i, j, l]
                                  * f_1[c_s_index + i] * f_1[c_s_index + l]
                                  * test[c_s_index + j] * dx(metadata={"quadrature_degree": 3}))
                F_c_s += ((1. / material.R_s) * (1. / F) * self.P[j] * f_1[j_li_index]
                          * test[c_s_index + j] * dx(metadata={"quadrature_degree": 2}))
                F_c_s_ret.append(F_c_s)
    ...
    def get_average_c_s(self, f, domain):
        ...
        c_s = []
        for i, materials in enumerate(domain.active_materials):
