...
from ufl import exp, inner, grad, SpatialCoordinate, Measure, TestFunction, TrialFunction, derivative
...
class StandardParticleIntercalation(WeakCoupledPM):
    ...
    def build_mesh(self, Ns):
        ...
        facet_tag = dfx.mesh.MeshTags(self.mesh, fdim, facet_indices[sorted_facets], facet_markers[sorted_facets])

        self.dx = Measure('dx', domain=self.mesh)
        self.ds = Measure('ds', domain=self.mesh, subdomain_data=facet_tag)

    def build_fs(self):
        P1 = dfx.fem.FunctionSpace(self.mesh, ('CG',1))
    ...
    def _update_constants(self, super_dof):
        timer0 = Timer('Update constants'); timer0.start()
    ...
    def _solve(self):
        ...
        for super_dof in range(db_shape[0]):
            ...
            for mat in range(db_shape[1]):
                timer1 = Timer('Update from db'); timer1.start()
...
class StressEnhancedIntercalation(StandardParticleIntercalation):
    def theta(self, material, R):
        if material.omega is not None and material.young is not None and material.poisson is not None:
            return (material.omega*2*material.young*material.omega)/(R*9*(1-material.poisson))/self.T
        else:
