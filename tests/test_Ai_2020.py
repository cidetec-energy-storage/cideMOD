...
def _run_case(
    ...
    for C_rate in [0.5, 1, 2]:
        save_path = init_results_folder(
            case, overwrite=overwrite, copy_files=[f"{data_path}/params.json", options.dict()], filenames=['params','simulation_options']
        )
...
def _check_results(
    ...
    if voltage:
        ...
        # _create_comparison(model_result.T,reference.T,"Time [s]","Voltage [V]",f"Ai_2020 @ {C_rate} C",problem.save_path)
    if temperature:
        ...
        error, time_ratio = _check_results_near(model_result, reference)
        # _create_comparison(model_result.T,reference.T,"Time [s]","Temperature [K]",f"Ai_2020 @ {C_rate} C",problem.save_path)
